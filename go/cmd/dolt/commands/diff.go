...
func parseDiffArgs(queryist cli.Queryist, sqlCtx *sql.Context, ctx context.Context, apr *argparser.ArgParseResults) (*diffArgs, error) {
	...
	tableSet, err := parseDiffTableSet(queryist, sqlCtx, ctx, dArgs.diffDatasets, tableNames)
	if err != nil {
		return nil, err
	}
...
func parseDiffTableSet(queryist cli.Queryist, sqlCtx *sql.Context, ctx context.Context, datasets *diffDatasets, tableNames []string) (*set.StrSet, error) {
...
func (dArgs *diffArgs) applyDiffRoots(queryist cli.Queryist, sqlCtx *sql.Context, ctx context.Context, args []string, isCached, useMergeBase bool) ([]string, error) {
	...
	currentTables, err := getTablesAtRef(queryist, sqlCtx, "HEAD")
	if err != nil {
		return nil, err
	}
	...
	if _, ok := currentTables[fromRef]; ok {
		// `dolt diff table`
		if useMergeBase {
			return nil, fmt.Errorf("Must supply at least one revision when using --merge-base flag")
		}
	...
	if _, ok := currentTables[toRef]; !ok {
...
func getCommonAncestor(queryist cli.Queryist, sqlCtx *sql.Context, c1, c2 string) (string, error) {
	//q := fmt.Sprintf("call dolt_common_ancestor('%s', '%s')", c1, c2)
	q := fmt.Sprintf("call dolt_merge_base('%s', '%s')", c1, c2)
	rows, err := getRowsForSql(queryist, sqlCtx, q)
	if err != nil {
		return "", err
	}
...
func diffUserTables(queryist cli.Queryist, sqlCtx *sql.Context, ctx context.Context, dArgs *diffArgs) errhand.VerboseError {
	...
	tableDeltas, err := diff.GetTableDeltas(queryist, sqlCtx, ctx, dArgs.fromRef, dArgs.toRef)
	if err != nil {
	...
	for _, td := range tableDeltas {
		...
		if isDoltSchemasTable(td) {
			...
			verr := diffUserTable(queryist, sqlCtx, td, dArgs, dw)
			if verr != nil {
				return verr
			}
		}
...
func diffUserTable(
	...
	if dArgs.diffParts&SchemaOnlyDiff != 0 {
		err := dw.WriteTableSchemaDiff(sqlCtx, dArgs.fromRoot, dArgs.toRoot, td)
		if err != nil {
			return errhand.VerboseErrorFromError(err)
		}
	}
	...
	verr := diffRows(queryist, sqlCtx, td, dArgs, dw)
	if verr != nil {
		return verr
	}
